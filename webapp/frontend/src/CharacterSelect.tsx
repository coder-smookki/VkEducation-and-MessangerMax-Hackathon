...
import { themeByImage } from './theme';               // 👈 добавь
import type { ImageKey } from './images';             // 👈 если нужен тип

// 🔹 ключ для хранения свечения
const glowKey = (id: string) => `lvlup_glow_${id}`;

// 🔹 хелпер: #RRGGBB -> rgba(r,g,b,a)
...
// 🔹 получаем пары haloFrom/haloTo для картинки
...
export default function CharacterSelect() {
  ...
  return (
    <div className="min-h-[100dvh] grid place-items-center px-4 py-6 md:py-4">
                ...
                <div key={c.id} className="px-2">
                  <CharacterCard
                    ...
                    onChoose={() => {
                      ...
                      localStorage.setItem(glowKey(c.id), JSON.stringify({ haloFrom, haloTo }));
                      nav(`/character/${c.id}`);
                    }}
                  />
                </div>
